...
- 信息明晰: 开发中有任何逻辑不理解的地方，直接询问开发者，不要猜测意图。

- 简洁导出: 代码文件内的 函数、常量、接口、类型 等申明及实现，遵循非必要不导出原则，也就是需要在外部引用到时才导出，没用到就不 export ，保持模块内私有的状态。

- 文件隔离: 以 -HumanOnly 或者 -HO 作为后缀的文件，是开发者用于记录操作或者分析思路的记事本，AI不要去读写。

- git提交: 所有 git 提交信息中，都不要包含 ai claude 等相关的署名信息，提交应该由当前的自然人开发者署名及负责。
