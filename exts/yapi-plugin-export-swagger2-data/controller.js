  ...
  async exportData(ctx) {
    ...
    try {
      ...
      switch (type) {
        ...
        { // in this time, only implemented OpenAPI V2.0
          const data = this.handleExistId(list)
          const model = await convertToSwaggerV2Model(data)
    ...
    async function convertToSwaggerV2Model(list) {
      const swaggerObj = {
        ...
        paths: (() => {
          const apisObj = {}
          for (const aptTag of list) { // list of category
            for (const api of aptTag.list) // list of api
            {
              ...
              apisObj[api.path][api.method.toLowerCase()] = (() => {
                ...
                apiItem.parameters = (() => {
                  const paramArray = []
                  for (const p of api.req_headers) // Headers parameters
                  {
                  ...
                  for (const p of api.req_params) // Path parameters
                  {
                    paramArray.push({
                      name: p.name,
                      in: 'path',
                  ...
                  for (const p of api.req_query) // Query parameters
                  {
                    paramArray.push({
                      name: p.name,
                      in: 'query',
