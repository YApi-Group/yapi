...
class wikiController extends baseController {
  ...
  async getWikiDesc(ctx) {
    try {
      ...
      const result = await this.Model.get(project_id)
  ...
  async uplodaWikiDesc(ctx) {
    try {
      ...
      // 如果当前数据库里面没有数据
      const result = await this.Model.get(params.project_id)
      if (!result) {
        ...
        const upRes = await this.Model.up(result._id, data)
  ...
  async wikiConflict(ctx) {
    try {
      ...
      ctx.websocket.on('message', async message => {
        ...
        const data = await this.websocketMsgMap(message, result)
        if (data) {
