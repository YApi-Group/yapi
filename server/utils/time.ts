 ...
 */
export function getDateRange(time = 90, start: string = undefined, withToday = true) {
...
// 时间
function convert2Decimal(num: number) { return num > 9 ? num : `0${num}` }
...
export function getDateInterval(time = 30) {
  // const gapTime = time * 24 * 3600 * 1000;
  // 今天
 ...
 */
export function getTimeInterval(date: string) {
 ...
 * 获取当前时间午夜0点的日期对象
 */
function getNowMidnightDate(time?: string) {
  let date
  if (time) {
    date = new Date(time)
  } else {
 ...
 */
export function formatYMD(val: any, joinStr = '-') {
  let date = val
  if (typeof val !== 'object') {
    date = new Date(val)
  }
 ...
 * @return {Number} 3
 */
export function getDayGapFromRange(dateRange: string[]) {
 ...
 */
function dateSpacialWithSafari(str: string) {
  if (str.indexOf('T') > -1) {
    let date: Date

    const [match, p1, p2, p3, p4, p5, p6] = /(\d{4})-(\d{2})-(\d{2})\w(\d{2}):(\d{2}):(\d{2})/.exec(str)
    if (match) { date = new Date(Number(p1), Number(p2) - 1, Number(p3), Number(p4), Number(p5), Number(p6)) }
...
export function transformSecondsToDay(seconds: number) {
