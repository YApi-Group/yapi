...
function mockValidator(interfaceData, ctx) {
  ...
  for (i = 0, l = interfaceData.req_query.length; i < l; i++) {
    const curQuery = interfaceData.req_query[i]
  ...
  try {
    ...
    if (!interfaceData || interfaceData.length !== queryPathInterfaceData.length) {
      ...
      for (i = 0, l = queryPathInterfaceData.length; i < l; i++) {
        match = false
        const currentInterfaceData = queryPathInterfaceData[i]
    ...
    if (!interfaceData || interfaceData.length === 0) {
      const newData = await interfaceInst.getVar(project._id, ctx.method)

      let findInterface
      const weight = 0
      _.each(newData, item => {
        const m = matchApi(newpath, item.path)
        if (m !== false) {
    ...
    try {
      ...
      if (context.resHeader && typeof context.resHeader === 'object') {
        for (const i of Object.keys(context.resHeader)) {
          let cookie
          if (i === 'Set-Cookie') {
