...
async function crossRequest(defaultOptions, preScript, afterScript, commonContext = {}) {
  ...
  const taskId = options.taskId || String(Math.random())
...
function handleParams(interfaceData, handleValue, requestParams) {
  const interfaceRunData = { ...interfaceData }
  ...
  if (HTTP_METHOD[interfaceRunData.method].request_body) {
    if (interfaceRunData.req_body_type === 'form') {
      requestBody = paramsToObjectWithEnable(safeArray(interfaceRunData.req_body_form).filter(item => item.type == 'text'))
    ...
    if (interfaceRunData.req_body_type === 'form') {
      requestOptions.files = paramsToObjectWithEnable(safeArray(interfaceRunData.req_body_form).filter(item => item.type == 'file'))
