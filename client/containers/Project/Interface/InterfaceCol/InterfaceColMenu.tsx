...
type PropTypes = {
  ...
  // fetchInterfaceCaseList?: AnyFunc
  // fetchInterfaceListMenu?: AnyFunc
  fetchCaseList?: AnyFunc
  fetchCaseData?: AnyFunc
  setColData?: AnyFunc
  currCaseId?: number
  history?: any
  isRander?: boolean
...
class InterfaceColMenu extends Component<PropTypes, StateTypes> {
  private _copyInterfaceSign = false
  ...
  addorEditCol = async () => {
    const { colName: name, colDesc: desc } = this.formRef.current.getFieldsValue()
  ...
  showDelColConfirm = (colId: number) => {
    ...
    confirm({
      ...
      onOk: async () => {
        ...
        if (!res.data.errcode) {
          ...
          this.props.history.push('/project/' + params.id + '/interface/col/' + nextColId)
        } else {
          message.error(res.data.errmsg)
        }
      },
  ...
  caseCopy = async (caseId: number) => {
    const caseData = await this.props.fetchCaseData(caseId)
  ...
  showDelCaseConfirm = (caseId: number) => {
    const params = this.props.match.params
    confirm({
      title: '您确认删除此测试用例',
      ...
      onOk: async () => {
        ...
        if (!res.data.errcode) {
          ...
          if (Number(caseId) === Number(this.props.currCaseId)) {
            this.props.history.push('/project/' + params.id + '/interface/col/')
          } else {
