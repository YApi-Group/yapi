  ...
  render() {
    ...
    const rowSelection = {
      ...
      onSelect: (record, selected) => {
        ...
        self.props.selectInterface(
          selectedRowKeys.filter(id => (String(id)).indexOf('category') === -1),
          self.state.project,
        )
      },
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: 'method',
        render: item => {
          const methodColor = variable.METHOD_COLOR[item ? item.toLowerCase() : 'get']
          return (
            <span
              style={{
      ...
      },
      {
        ...
        onFilter: (value, record) => {
          const arr = record.children.filter(item => item.status.indexOf(value) === 0)
          return arr.length > 0
